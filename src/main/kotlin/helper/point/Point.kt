...
@JvmName("strFindAll")
fun List<String>.findAll(char: Char): Set<Point> = flatMapIndexedTo(mutableSetOf()) { y: Int, row: String ->
...
fun <E> List<Collection<E>>.findAll(item: E): Set<Point> = flatMapIndexedTo(mutableSetOf()) { y, row ->
...
fun <E> Array<Array<E>>.indexOf(item: E): Point =
    this.mapIndexedNotNull { y, line -> line.indexOf(item).let { x -> if (x != -1) Point(x, y) else null } }.first()
fun <E> Array<Array<E>>.findAll(item: E): Set<Point> = flatMapIndexedTo(mutableSetOf())  { y, row ->
    row.mapIndexedNotNull { x, c -> if (c == item) Point(x, y) else null }
}
